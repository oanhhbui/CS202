...
var defaultWorkoutPlan = {
	prompt: "No details? No problem. Try this 3-day starter:",
	days: [
	  {day: 1, focus: "Full-body strength", type: "bodyweight or weights"},
	  {day: 2, focus: "Cardio", type: "brisk walk, bike, or HIIT"},
	  {day: 3, focus: "Core & mobility", type: "planks, stretching, yoga"}
	],
	followUp: "Want a version with sets and reps?"
  };
  
  function getDefaultPlan() {
	let response = defaultWorkoutPlan.prompt + "\n";
	defaultWorkoutPlan.days.forEach(day => {
	  response += `• Day ${day.day}: ${day.focus} (${day.type})\n`;
	});
	response += defaultWorkoutPlan.followUp;
	return response;
  }